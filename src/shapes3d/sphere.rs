                ...
                (0..=circle_steps)
                    ...
                    .map(move |cs| -> (f64, f64, f64) {
                        ...
                        (
                            ...
                            self.radius * (std::f64::consts::PI * cir).sin() * (std::f64::consts::TAU * rot).cos() + self.center.1,
                            self.radius * (std::f64::consts::PI * cir).sin() * (std::f64::consts::TAU * rot).sin() + self.center.2,
                        )
                    })
            })
            .collect()
