...
fn rotate(
    (x, y): (i32, i32),
    (x_center, y_center): (i32, i32),
    angle_rads: f32,
) -> (i32, i32, f64) {
    (
        ((x - x_center) as f32 * angle_rads.cos() - (y - y_center) as f32 * angle_rads.sin())
            as i32
            + x_center,
        ((x - x_center) as f32 * angle_rads.sin() + (y - y_center) as f32 * angle_rads.cos())
            as i32
            + y_center,
        0.0,
    )
...
fn amongus<const W: usize, const H: usize>(
    img: &mut Image<W, H>,
    (x, y): (i32, i32),
    ...
    let color = color!(
        (brightness_lerp * (255f32 - color_lerp)) as u8,
        (brightness_lerp * color_lerp) as u8,
        0
    );
    let lerped_cyan = color!(
        0,
        (brightness_lerp * 255f32) as u8,
        (brightness_lerp * 255f32) as u8
    );
    ...
    amongus(
        ...
        (x, y),
        (
            width - width / (iters + 1) as i32,
            height - height / (iters + 1) as i32,
        ),
        angle_rads + std::f32::consts::PI / (total_iters as f32),
        iters - 1,
        total_iters,
    );
}
...
fn dw_test() {
    ...
    img.draw_line(
        (0, 0, 0.0),
        (xresint - 1, yresint - 1, 0.0),
        color_constants::GREEN,
    );
    img.draw_line(
        (0, 0, 0.0),
        (xresint - 1, yresint / 2, 0.0),
        color_constants::GREEN,
    );
    img.draw_line(
        (xresint - 1, yresint - 1, 0.0),
